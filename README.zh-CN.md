...
- 用户级 Codex `Stop`、`PermissionRequest` 与 `PostToolUse` Hook，不覆盖现有 `notify`。
- 支持完成、权限、回复、确认和需要处理通知，并采用保守的本地文本分类。
...
- 完成通知只表示一个 Codex 回合已结束，不表示整个任务完成，也不提供真实进度。权限请求提醒默认关闭，因为 Codex Hook 不会暴露请求是由 Auto-review 还是人工处理；“始终提醒”会报告每个唯一请求，也可能包含最终由 Codex 自动处理的请求。其他需要操作的自然语言分类刻意保持保守，不能保证识别所有请求。
...
参见 [贡献指南](CONTRIBUTING.md)、[故障排除](docs/TROUBLESHOOTING.md)、[M6 人工测试](docs/M6_MANUAL_TEST.md)、[国际化测试指南](docs/localization/testing.md) 和 [发布清单](docs/M5_RELEASE_CHECKLIST.md)。Release Dry Run 不会创建 tag 或 GitHub Release。
